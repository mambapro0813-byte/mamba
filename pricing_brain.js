...
(function(){
 const KEY='ai_trade_pricing_brain_v2',CORE='aitrade_full59_v1';
 ...
 function num(v){v=parseFloat(String(v??'').replace(/[^0-9.\-]/g,''));return Number.isFinite(v)?v:0}
 function products(){return window.PRODUCTS||window.products||[]}
 function core(){try{return JSON.parse(localStorage.getItem(CORE)||'null')||{customers:[]}}catch{return{customers:[]}}}
 function product(model){return products().find(p=>String(p.model||p.sku||'').toLowerCase()===String(model||'').toLowerCase())}
 ...
 function calc(input){
  ...
  const margin=cost&&unit?(unit-cost)/unit:null, flags=[];
  if(!ref) flags.push('产品参考价缺失');
  if(!cost) flags.push('成本价缺失');
  if(ref&&unit<ref&&cfg.approval.belowReference) flags.push('低于产品参考价');
  if(margin!==null&&margin<cfg.warningMargin&&cfg.approval.belowWarningMargin) flags.push('毛利低于预警线');
  if(input.paymentTerms&&String(input.paymentTerms).toLowerCase()!=='30% deposit + 70% before shipment'&&cfg.approval.paymentTerms) flags.push('特殊付款条件');
  if(input.exclusive&&cfg.approval.exclusive) flags.push('独家/区域代理条件');
  if(input.mold&&cfg.approval.mold) flags.push('模具/开发费用');
  if(custom>0&&cfg.approval.customization) flags.push('定制费用');
 ...
 function quotation(input){
  const r=calc(input), c=(core().customers||[]).find(x=>String(x.id)==String(input.customerId));
 ...
 window.PricingBrain={cfg,save,calc,quotation,product,products,core};
})();