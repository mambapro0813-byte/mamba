...
function renderRfq(){let sec=document.getElementById('quotes');if(!sec)return;let q=read(RFQ,{items:[]}).items||[],box=document.getElementById('rfqAutoQueue');if(!box){box=document.createElement('div');box.id='rfqAutoQueue';box.className='card';box.style.marginBottom='10px';let table=sec.querySelector('.card');sec.insertBefore(box,table||null)}box.innerHTML=`<div class="top" style="margin-bottom:8px"><div><b>RFQ 自动报价队列</b><div class="sub">邮件识别为 RFQ 后自动进入这里；价格仍保留人工审批。</div></div><span class="tag ${q.length?'warn':'ok'}">${q.length} 条</span></div>${q.length?`<div class="wrap"><table><thead><tr><th>客户</th><th>型号</th><th>数量</th><th>邮件主题</th><th>状态</th></tr></thead><tbody>${q.slice(0,30).map(x=>`<tr><td><b>${esc(x.company)}</b></td><td>${esc(x.model||'待确认')}</td><td>${x.qty||'待确认'}</td><td>${esc(x.subject||'-')}</td><td><span class="tag warn">待补价格 / 审批</span></td></tr>`).join('')}</tbody></table></div>`:'<div class="item">暂无RFQ。客户邮件出现 price / quote / RFQ / precio / cotización 等意图后会自动进入。</div>`}`;
